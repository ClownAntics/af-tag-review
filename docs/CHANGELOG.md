...
- **Review tools for catalog audits: random sample mode + mistag detector.**
  ...
  - **Mistag audit** section in Settings re-runs Claude vision on a random sample of `Updated` designs and surfaces ones where the fresh primary tag disagrees with the stored `vision_raw.primary`. `/api/review/mistag-audit?count=N` streams NDJSON for live progress. Each suspect renders with its thumbnail, design name, stored→new primary diff, and Claude's new reasoning. A per-suspect `⚑ Flag for re-review` button sends the design back through the pipeline. Read-only otherwise. ~$0.006/design, default count 20 ≈ $0.12/run. Cheap enough to run weekly.
