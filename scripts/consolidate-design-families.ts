...
async function applyMerge(
  ...
  const merged = {
    status: winnerStatus,
    variant_skus: unionArr(winner.variant_skus, loser.variant_skus),
    shopify_product_ids: unionNumArr(
      winner.shopify_product_ids,
      loser.shopify_product_ids,
    ),
    shopify_product_types: unionArr(
      winner.shopify_product_types,
      loser.shopify_product_types,
    ),
    shopify_tags: unionArr(winner.shopify_tags, loser.shopify_tags),
    approved_tags: approved,
    vision_tags: vision,
    image_url: winner.image_url ?? loser.image_url ?? null,
  };
...
async function applyRename(
  ...
  if (updErr)
    throw new Error(`rename ${current.design_family} → ${canonical}: ${updErr.message}`);
